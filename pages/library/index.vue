<template>
  <v-content>
    <v-container>
        ...
        <div class="col-12 col-sm-6 col-md-3">
          <v-dialog v-model="dialogCreateLib" scrollable max-width="300px">
            ...
            <v-card>
              ...
              />
              <v-card-actions>
                <v-btn color="brown darken-1" text @click="dialogCreateLib = false">Close</v-btn>
                <v-btn
          ...
          <v-dialog v-model="dialogDeleteLib" scrollable max-width="300px">
            ...
            <v-card>
              ...
              <v-card-text style="height: 300px;">
                <v-radio-group v-if="libraries.length" v-model="dialogDeleteLibModel" column>
                  <v-radio :label="lib.id" :value="lib.id" v-for="lib in libraries" :key="lib.id"></v-radio>
                </v-radio-group>
              </v-card-text>
              <v-card-actions>
                <v-btn color="brown darken-1" text @click="dialogDeleteLib = false">Close</v-btn>
                <v-btn
                  color="brown darken-1"
        ...
        <v-flex class="col-12" v-if="libraries.length">
          <v-card>
            <v-tabs v-model="tab" color="brown" grow show-arrows>
              ...
              <v-tab @click="changeTab" v-for="lib in libraries" :key="lib.id">{{ lib.id }}</v-tab>
            </v-tabs>
          </v-card>
        </v-flex>
      </v-flex>
      <v-flex v-if="currentLibId !== null" row justify-space-around class="ma-0">
        <!-- add begin -->
        <v-dialog v-model="dialogAddCol" scrollable max-width="300px">
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" class="d-none d-sm-none d-md-flex" color="light-green lighten-1">
              <v-icon left>mdi-plus</v-icon>Add Collection
            </v-btn>
            <v-btn v-on="on" class="d-sm-flex d-md-none col-6" color="light-green lighten-1">
          ...
          <v-card>
            ...
            <v-card-text class="d-flex" style="height: 300px;">
              ...
              <div v-else class="d-flex display-1 align-center text-center justify-center row">
                <div class="d-flex">No more collections left for adding to this library :(</div>
            ...
            <v-card-actions>
              <v-btn color="brown darken-1" text @click="dialogAddCol = false">Close</v-btn>
              <v-btn
                color="brown darken-1"
        ...
        <v-dialog v-model="dialogRemoveCol" scrollable max-width="300px">
          <template v-slot:activator="{ on }">
            ...
            <v-btn v-on="on" class="d-block d-md-none col-6 overflow-hidden" color="red lighten-3">
          ...
          <v-card>
            ...
            <v-card-text class="d-flex" style="height: 300px;">
              <v-radio-group
                ...
                column
              >
                <v-radio :label="col" :value="col" v-for="col in libraryCollections()" :key="col"></v-radio>
              </v-radio-group>
              <div v-else class="d-flex display-2 align-center text-center">nothing to remove :(</div>
            </v-card-text>
            <v-card-actions>
              <v-btn color="brown darken-1" text @click="dialogRemoveCol = false">Close</v-btn>
              <v-btn
                color="brown darken-1"
      ...
      <hlib-collection-list v-if="libraries.length" :collectionIdList="collectionIdList" />
...
export default {
  ...
  methods: {
    collectionsNotInLibrary() {
      if (this.currentLibId === null) return [];
      const allColls = this.$store.getters['collection/getAll'].map(c => c.id);
