<template>
  <v-content>
    <v-layout>
      <v-flex column>
        <v-img :src="$books.getCover({ id: bookId })" contain aspect-ratio="1" max-height="300px" />
        <div class="d-flex justify-center">
          <div
            class="d-none d-sm-none d-md-flex justify-center display-1 text-center col-md-8"
          >{{ book.title }}</div>
          <div
            class="d-flex d-sm-flex d-md-none justify-center title text-center col-12 col-sm-10 col-md-8"
          >{{ book.title }}</div>
        </div>
        <v-dialog v-model="dialog" scrollable max-width="300px">
          <template v-slot:activator="{ on }">
            <v-flex justify-center row class="ma-0">
              <v-btn
          ...
          <v-card>
            ...
            <v-card-text class="d-flex" style="height: 300px;">
              ...
              >Nothing to choose here :(</div>
            </v-card-text>
            <v-card-actions>
              <v-btn color="brown darken-1" text @click="dialog = false">Close</v-btn>
              <v-btn
        ...
        <v-snackbar color="success" right bottom v-model="snackbarSuccess">
          Book was successfully added to
          <nuxt-link class="px-1" :to="collectionLink">{{ colId }}</nuxt-link>collection
        </v-snackbar>
        <v-flex row class="ma-0 py-3">
          <div class="d-flex font-weight-medium pr-3">Authors:</div>
          <div class="d-flex pr-3" v-for="author in book.authors" :key="author">{{ author }}</div>
        </v-flex>
        <v-flex class="ma-0">
        ...
        <v-simple-table>
          <tbody>
            <tr v-for="pair in bookMetaTable" :key="pair[0]">
              <td>{{ pair[0] }}</td>
              <td>{{ pair[1] }}</td>
            </tr>
          </tbody>
